...
class AID(sb.Brain):
    ...
    def init_optimizers(self):
        ...
        if hparams["use_adapters"]:
            ...
            # Load the config from the model folder or file directly
            peft_config = PeftConfig.from_pretrained(hparams["adapter_model_path"])
            hparams["wav2vec2"] = PeftModel.from_pretrained(hparams["wav2vec2"], config=peft_config)
        else:
            self.wav2vec2_optimizer = self.hparams.wav2vec2_opt_class(
                self.modules.wav2vec2.parameters()
            )
...
if __name__ == "__main__":
    ...
    if hparams["use_adapters"]:
        from peft import PeftModel, PeftConfig
        peft_config = PeftConfig(adapter_model=hparams["adapter_model_path"])
        hparams["wav2vec2"] = PeftModel.from_pretrained(hparams["wav2vec2"], peft_config)
